...
def MAIN():
    ...
    if pb.Base_Model == "TextCNN":
        ...
        if os.path.exists(w2v_pickle) == False:
            if pb.Dataset_Name in ["Suning", "Taobao"]:
                wv, word2id = (
                    KeyedVectors.load_word2vec_format(w2v_path, binary=False),
                    {},
                )
            else:
                wv, word2id = (
                    KeyedVectors.load_word2vec_format(
                        w2v_path, binary=False, no_header=True
                    ),
                    {},
                )
        ...
        train_dataset = MyDataset_TextCNN(
            embedding, word2id, myAllDataset.train_examples
        )
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=pb.Train_Batch_Size, shuffle=pb.DataLoader_Shuffle
    )
    dev_loader = DataLoader(
        dev_dataset, batch_size=pb.DevTest_Batch_Size, shuffle=pb.DataLoader_Shuffle
    )
    test_loader = DataLoader(
        test_dataset, batch_size=pb.DevTest_Batch_Size, shuffle=pb.DataLoader_Shuffle
    )
